...
function eventsFromBoxScoreLine(line) {
  const out = [];
  const push = (n, type, points) => {
    for (let i = 0; i < n; i++) out.push(makeEvent(type, { player: line.nom, points }));
  };
  ...
  push(two.made, EVENT_TYPES.SHOT_MADE, 2);
  push(two.attempted - two.made, EVENT_TYPES.SHOT_MISSED, 0);
  push(three.made, EVENT_TYPES.SHOT_MADE, 3);
  push(three.attempted - three.made, EVENT_TYPES.SHOT_MISSED, 0);
